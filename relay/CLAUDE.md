...
`ADMIN_TOKEN` — read from the environment via the `fromEnvironment` binding in config.capnp. In production it lives in `/etc/cute-relay.env` (mode 600, generated by setup.sh). Never commit. Used to protect the `/register` endpoint — it doubles as the "workshop code" participants type into the flash tool, so it's a human-readable phrase (`fuzzy-otter-disco-42`) generated by `deploy/new-token.sh`. Rotate with `sudo ./deploy/new-token.sh --install` on the server (restarts the relay; the registry survives restarts since it's persisted to disk).
