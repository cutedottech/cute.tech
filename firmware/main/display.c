...
/* ── Pin definitions (Heltec WiFi LoRa 32 V4) ───────────────────────────── */
#define OLED_SDA   17
#define OLED_SCL   18
#define OLED_RST   21
#define OLED_VEXT  36
...
static uint8_t fb[OLED_W * OLED_PAGES]; /* 1 KB framebuffer */
...
{
    i2c_master_transmit(oled_dev, buf, len, 500); /* 500 ms — parameter is ms, not ticks */
}
...
static void oled_flush(void)
{
    ...
    /* Stream framebuffer one page at a time (128 data bytes + control byte) */
    uint8_t row[OLED_W + 1];
    row[0] = 0x40; /* data stream */
...
{
    /* Enable display power — VEXT active LOW on Heltec boards */
    gpio_config_t io = {
        .pin_bit_mask = (1ULL << OLED_VEXT) | (1ULL << OLED_RST),
        .mode         = GPIO_MODE_OUTPUT,
    };
    gpio_config(&io);
    gpio_set_level(OLED_VEXT, 0); /* turn VEXT on */
...
{
    fb_clear();
    fb_draw_str(0, 1, "needs setup");
    fb_draw_str(0, 3, "open cute.tech");
    fb_draw_str(0, 4, "to configure");
    oled_flush();
}
...
void display_show_connecting(const char *ssid)
{
    char line[22];
    fb_clear();
    fb_draw_str(0, 0, "connecting...");
    snprintf(line, sizeof(line), "%.12s", ssid);
    fb_draw_str(0, 2, line);
    oled_flush();
}
...
void display_show_connected(const char *ip)
{
    char line[22];
    fb_clear();
    fb_draw_str(0, 0, "wifi connected");
    snprintf(line, sizeof(line), "%.17s", ip);
    fb_draw_str(0, 2, line);
    oled_flush();
}
...
void display_show_disconnected(int reason)
{
    (void)reason;
    fb_clear();
    fb_draw_str(0, 0, "wifi lost");
    fb_draw_str(0, 2, "retrying...");
    oled_flush();
}
...
void display_show_online(const char *device_name)
{
    char line[22];
    fb_clear();
    fb_draw_str(0, 0, "online");
    snprintf(line, sizeof(line), "%.12s", device_name);
    fb_draw_str(0, 2, line);
    fb_draw_str(0, 3, ".cute.tech");
    oled_flush();
}
